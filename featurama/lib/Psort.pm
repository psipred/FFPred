...
sub parse
{
    ...
    my $RES = {}; # This will contain 12 features (below; see also WoLF-PSORT's runWolfPsortSummary man page & http://wolfpsort.org/aboutWoLF_PSORT.html.en).
    ...
    {
        $RES->{$type} = $CFG->{$type_list->{$type}}{'val'} = 0;
    ...
    if (-s "$self->{path}/$self->{md5}.wolfpsort")
    {
        open(PSORT, "$self->{path}/$self->{md5}.wolfpsort");
        ...
        if ($k_line =~ /k used for kNN is:\s*(\d+)/)
        {
            ...
            foreach my $result ($results_line =~ /\b(?:$reg_exp)\s+\d+(?:\.\d+)?/g)
            {
                ...
                $value /= $self->{'k'};
                $RES->{$type} = $CFG->{$type_list->{$type}}{'val'} = $value;
            }
        }
    }
